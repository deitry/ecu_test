...
void EC_Engine::setQCTrans(void)
{
	if (fabs(err) > EG::dZone)
	{
		...
		{
			// зона пониженной чувствительности - для улучшения стабильности
			QC = PIDstab*(kP*err + kD*errD) + kI*errI; // интегральная составляющая корректируется за счёт снижения темпа набора errI
