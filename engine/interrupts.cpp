...
__interrupt void xint1_isr(void)
{
			...
			&& (zCnt <= (DIESEL_Z_ALL - 1))) 	// если пропуска зуба не обнаружено
	{
		...
		nR._val = omegaR*30/PI;	// пересчёт в об/мин

		errN = nU - nR._val;		// текущая ошибка
...
__interrupt void cpu_timer1_isr(void)
{
	...
	if (manInj)
	{
		...
		for (i1 = 0; i1 < DIESEL_N_CYL; i1++)
		{
			...
			{
				injCnt[i1]++;
				if (injCnt[i1] >= dTime[i1]/TIM1_DIV) // && (QC > QCmin) // -2 - впрыск на 200 мкс раньше, чем "планируется"
